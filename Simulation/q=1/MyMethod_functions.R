...
OraProjInt_SE_popu_noX = function(theta0, theta.T, theta.G, Theta.D,
  ...
  #' The theta's can have different nonzero entry locations, not limited to heredity
  #' @return NIE_SE NDE_SE are the matrices of SE of the NIE and NDE for different j
  ...
  e1 = matrix(0, nrow=(1+q), ncol=1)
  e1[1,1] = 1
  
  e2 = matrix(0, nrow=(1+q), ncol=1)
  e2[2,1] = 1
  
  alpha = etilde.2 + t(E_AD1)%*%beta0[A_D[[1]],1]
  
  var.NDE0 = (sigma_Y^2) * (t(alpha)%*%Sigma.inv%*%alpha) + ( t(e1)%*%Sigma11.inv%*%(e1) ) * ( t(Theta.D[A_D[[1]],1])%*%Sigma_G[ A_D[[1]],A_D[[1]] ]%*%Theta.D[A_D[[1]],1] )
  NDE_SE[1, "Tj=0"] = sqrt(var.NDE0)
  rm(var.NDE0)
  
  var.NDE1 = (sigma_Y^2) * ( t(alpha + t(E_AD1)%*%B[A_D[[1]],1])%*%Sigma.inv%*%(alpha + t(E_AD1)%*%B[A_D[[1]],1]) ) + 
    ( t(e1 + e2)%*%Sigma11.inv%*%(e1 + e2) ) * ( t(Theta.D[A_D[[1]],1])%*%Sigma_G[A_D[[1]],A_D[[1]]]%*%Theta.D[A_D[[1]],1] )
  NDE_SE[1, "Tj=1"] = sqrt(var.NDE1)
  rm(var.NDE1)
  ...
  if(q > 1){
    for (j in 2:q) {
      ...
      ejp1 = matrix(0, nrow=(1+q), ncol=1)
      ejp1[(j+1),1] = 1
      
      alpha = etilde.jp1 + t(E_ADj)%*%beta0[A_D[[j]],1]
      
      var.NDE0 = (sigma_Y^2) * (t(alpha)%*%Sigma.inv%*%alpha) + ( t(e1)%*%Sigma11.inv%*%(e1) ) * ( t(Theta.D[A_D[[j]],j])%*%Sigma_G[ A_D[[j]],A_D[[j]] ]%*%Theta.D[A_D[[j]],j] )
      NDE_SE[j, "Tj=0"] = sqrt(var.NDE0)
      rm(var.NDE0)
      
      var.NDE1 = (sigma_Y^2) * ( t(alpha + t(E_ADj)%*%B[A_D[[j]],j])%*%Sigma.inv%*%(alpha + t(E_ADj)%*%B[A_D[[j]],j]) ) + 
        ( t(e1 + ejp1)%*%Sigma11.inv%*%(e1 + ejp1) ) * ( t(Theta.D[A_D[[j]],j])%*%Sigma_G[A_D[[j]],A_D[[j]]]%*%Theta.D[A_D[[j]],j] )
      NDE_SE[j, "Tj=1"] = sqrt(var.NDE1)
      rm(var.NDE1)
